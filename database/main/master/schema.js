...
const Schema = function (logger, executor, configMain) {
  ...
  // Check if CoinChanges Table Exists in Database
  this.selectCoinChanges = function(pool, callback) {
    const command = `
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_schema = '${ pool }'
        AND table_name = 'coin_changes');`;
    _this.executor([command], (results) => callback(results.rows[0].exists));
  };

  // Deploy CoinChanges Table to Database
  this.createCoinChanges = function(pool, callback) {
    const command = `
      CREATE TABLE "${ pool }".coin_changes(
        id BIGSERIAL PRIMARY KEY,
        change FLOAT NOT NULL DEFAULT 0,
        interval VARCHAR NOT NULL DEFAULT 'unknown',
        type VARCHAR NOT NULL DEFAULT 'primary',
        CONSTRAINT coin_change_unique UNIQUE (interval, type));`;
    _this.executor([command], () => callback());
  };

  // Check if CoinPrices Table Exists in Database
  this.selectCoinPrices = function(pool, callback) {
    const command = `
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_schema = '${ pool }'
        AND table_name = 'coin_prices');`;
    _this.executor([command], (results) => callback(results.rows[0].exists));
  };

  // Deploy CoinPrices Table to Database
  this.createCoinPrices = function(pool, callback) {
    const command = `
      CREATE TABLE "${ pool }".coin_prices(
        id BIGSERIAL PRIMARY KEY,
        currency VARCHAR NOT NULL DEFAULT 'unknown',
        price FLOAT NOT NULL DEFAULT 0,
        type VARCHAR NOT NULL DEFAULT 'primary',
        CONSTRAINT coin_price_unique UNIQUE (currency, type));`;
    _this.executor([command], () => callback());
  };
  ...
  this.createCurrentMiners = function(pool, callback) {
    const command = `
      CREATE TABLE "${ pool }".current_miners(
        ...
        active_solo INT NOT NULL DEFAULT 0,
        ...
        inactive_solo INT NOT NULL DEFAULT 0,
  ...
  this.createCurrentWorkers = function(pool, callback) {
    const command = `
      CREATE TABLE "${ pool }".current_workers(
        ...
        effort FLOAT NOT NULL DEFAULT 0,
        hashrate FLOAT NOT NULL DEFAULT 0,
        average_hashrate FLOAT NOT NULL DEFAULT 0,
        identifier VARCHAR NOT NULL DEFAULT 'master',
        invalid INT NOT NULL DEFAULT 0,
  ...
  this.handleDeployment = function(pool) {
    return new Promise((resolve) => {
      _this.handlePromises(pool, _this.selectSchema, _this.createSchema)
        .then(() => _this.handlePromises(pool, _this.selectCoinChanges, _this.createCoinChanges))
        .then(() => _this.handlePromises(pool, _this.selectCoinPrices, _this.createCoinPrices))
