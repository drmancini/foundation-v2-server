...
const Rounds = function (logger, client, config, configMain) {
  ...
  this.handleCurrentMetadata = function(metadata, updates, share, shareType, minerType, blockType) {
    ...
    const curerntEffort = filteredMetadata.reduce((a, b) => a.effort + b, 0);
    const work = (filteredMetadata.work || 0) + (updates.work || 0);
    const newEffort = shareType === 'valid' ? _this.handleEffort(share, difficulty, work, shareType) : curerntEffort;

    // Calculate Effort Metadata
    const effort = minerType ? 0 : newEffort;
  ...
  this.handleMetadataHistory = function(shares, blockType) {
    ...
    shares.forEach((share) => {
      ...
      metadata[unique] = _this.handleHistoricalMetadata(current, share, shareType, minerType, blockType);
    });
  ...
  this.handleMiners = function(shares, blockType) {
    ...
    shares.forEach(share => {
      ...
      if (minerType)
        miners[unique] = _this.handleCurrentMiners(current, share, shareType, minerType, blockType);
    });
  ...
  this.handleShares = function(rounds, shares, blockType) {
    ...
    shares.forEach((share) => {
      ...
      updates[unique] = _this.handleCurrentRounds(initial, current, share, ipHash, shareType, minerType, blockType);
    });
  ...
  this.handleWorkersHistory = function(shares, blockType) {
    ...
    shares.forEach((share) => {
      ...
      workers[unique] = _this.handleHistoricalWorkers(current, share, ipHash, shareType, minerType, blockType);
    });
  ...
  this.handleCleanup = function(segment, callback) {
    ...
    const segmentDelete = segment.map(segment => `'${ segment.uuid }'`);
    const transaction = [
      'BEGIN;',
