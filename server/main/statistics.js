...
const Statistics = function (logger, client, config, configMain, template) {
  ...
  this.handleCurrentMetadata = function(miners, workers, total) {
  ...
  this.handleCurrentMiners = function(hashrate, miners) {
  ...
  this.handleCurrentWorkers = function(hashrate, workers) {
  ...
  this.handlePrimary = function(lookups, callback) {
    ...
    if (lookups[2].rows[0] && lookups[3].rows[0] && lookups[4].rows[0] && lookups[8].rows[0]) {
      ...
      const metadataUpdates = _this.handleCurrentMetadata(
        minersMetadata, workersMetadata, currentMetadata);
    ...
    if (lookups[11].rows.length >= 1) {
      ...
      const minersUpdates = _this.handleCurrentMiners(hashrate, miners);
    ...
    if (lookups[15].rows.length >= 1) {
      ...
      const soloWorkersUpdates = _this.handleCurrentWorkers(hashrate, soloWorkers);
    ...
    if (lookups[16].rows.length >= 1) {
      ...
      const sharedWorkersUpdates = _this.handleCurrentWorkers(hashrate, sharedWorkers);
      transaction.push(_this.current.workers.insertCurrentWorkersHashrate(
  ...
  this.handleStatistics = function(callback) {

    // Handle Initial Logging
    const starting = [_this.text.databaseStartingText1()];
    ...
    const transaction = [
      ...
      _this.current.hashrate.countCurrentHashrateMiner(_this.pool, hashrateWindow, 'primary'),
      _this.current.hashrate.countCurrentHashrateWorker(_this.pool, hashrateWindow, true, 'primary'),
      _this.current.hashrate.countCurrentHashrateWorker(_this.pool, hashrateWindow, false, 'primary'),
      _this.current.hashrate.sumCurrentHashrateMiner(_this.pool, hashrateWindow, 'primary'),
      _this.current.hashrate.sumCurrentHashrateWorker(_this.pool, hashrateWindow, true, 'primary'),
      _this.current.hashrate.sumCurrentHashrateWorker(_this.pool, hashrateWindow, false, 'primary'),
      _this.current.hashrate.sumCurrentHashrateType(_this.pool, hashrateWindow, false, 'primary'),
      ...
      _this.current.miners.selectCurrentMinersMain(_this.pool, { type: 'primary' }),
      _this.current.network.selectCurrentNetworkMain(_this.pool, { type: 'primary' }),
      ...
      _this.current.workers.selectCurrentWorkersMain(_this.pool, { solo: true, type: 'primary' }),
      _this.current.workers.selectCurrentWorkersMain(_this.pool, { solo: false, type: 'primary' }),
      'COMMIT;'];

    // Perform SQL Queries
    _this.executor(transaction, (lookups) => {
      _this.handlePrimary(lookups, () => {
        const updates = [_this.text.databaseUpdatesText1()];
  ...
  this.handleInterval = function() {
    ...
    setTimeout(() => {
      ...
      if (_this.config.primary.checks.enabled) _this.handleStatistics(() => {});
    }, random);
  };
