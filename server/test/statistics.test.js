...
describe('Test statistics functionality', () => {
  ...
  test('Test statistics handleCurrentWorkers updates [3]', () => {
    MockDate.set(1634742080841);
    const client = mockClient(configMainCopy, { rows: [] });
    const logger = new Logger(configMainCopy);
    const template = { algorithms: { sha256d: { multiplier: 1 }}};
  ...
  test('Test statistics handleCurrentWorkerAggregates updates [1]', () => {
    MockDate.set(1634742080841);
    const client = mockClient(configMainCopy, { rows: [] });
    const logger = new Logger(configMainCopy);
    const template = { algorithms: { sha256d: { multiplier: 1 }}};
    const statistics = new Statistics(logger, client, configCopy, configMainCopy, template);
    const workers = [
      { miner: 'miner1', worker: 'miner1', average_hashrate: 1, invalid: 0, solo: false, stale: 0, valid: 1 },
      { miner: 'miner2', worker: 'miner2', average_hashrate: 0, invalid: 0, solo: false, stale: 0, valid: 1 }];
    const expected = [{
      timestamp: 1634742080841,
      miner: 'miner1',
      worker: 'miner1',
      ...
      type: 'primary',
      valid: 1}];
    expect(statistics.handleCurrentWorkerAggregates(workers, 'primary')).toStrictEqual(expected);
  });
